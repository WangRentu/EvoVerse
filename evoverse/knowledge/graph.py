    ...
    def _ensure_container_running(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "ps", "--filter", "name=evoverse-neo4j", "--format", "{{.Names}}"],
                capture_output=True,
                text=True,
                timeout=10
            )

            if "evoverse-neo4j" in result.stdout:
            ...
            for i in range(max_retries):
                try:
                    result = subprocess.run(
                        ["docker", "exec", "evoverse-neo4j", "cypher-shell",
                         "-u", "neo4j", "-p", "evoverse-password", "RETURN 1"],
                        capture_output=True,
                        timeout=5
                    )
