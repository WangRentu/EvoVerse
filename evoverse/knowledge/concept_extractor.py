"""
LLM-powered concept and method extraction from scientific papers.

Uses EvoVerse LLM (Qwen via vLLM) to extract structured information including:
...
class ConceptExtractor:
    """
    LLM-powered extractor for scientific concepts and methods.

    Uses EvoVerse LLM (Qwen via vLLM) with specialized prompts to extract structured information
    ...
    def __init__(
        ...
        Args:
            llm_client: LLMClient instance (default: create new from config)
        ...
        # LLM setup - use provided client or create new one
        if llm_client is None:
            self.llm_client = LLMClient(max_history=0)  # No conversation history needed
        ...
        # Use model from client or config
    ...
    def _extract_concepts_and_methods(
        ...
        """
        Extract concepts and methods using LLM.
        ...
        # Call LLM API
        try:
            # Use LLMClient's chat method with single message
            messages = [{"role": "user", "content": prompt}]
            
            # Temporarily override max_tokens and temperature for this call
            original_max_tokens = self.llm_client.max_tokens
            original_temperature = self.llm_client.temperature
            
            self.llm_client.max_tokens = self.max_tokens
